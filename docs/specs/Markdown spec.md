...
#### Inline templates and HTML

Inline templates and HTML, that means HTML and templates within a paragraph, are handled somewhat differently.

* HTML is not handled separately. Paragraphs with HTML inside are subject to markdown parsing. This means that any markdown elements need to be escaped if you mix HTML inside a paragraph, and then mix markdown characters illegally within parts of it.
* Templates are identified and inserted literally. Text inside `{{ templates }}` or `{% templates %}` are inserted literally, so content inside skips markdown parsing

Inline HTML may be skipped in the future, and this will be denoted as a breaking change in the changelog. However, this will require an HTML parser that violet does not currently have.
...
IDs are generated by lower-casing characters, only keeping non-latin letters and numbers (and - and \_), and replacing spaces with `-`. In the event of duplicate IDs, disambiguation is done with `-{number}`. For example, given
```markdown
## C
## C++
## C#
...
Striking out text is implemented with `~~strike~~` (~~strike~~). This is rendered as `<s>strike</s>`. If you need `<del>` specifically, use the raw HTML tag.
...
Links literally included in the text are not currently rendered. Doing so may or may not be implemented in the future. This is because implementing inline links would require markup to override the automatic link detection, which itself can result in extra requirements for URL encoding certain links to retain expected behaviour, which is annoying to implement.

Instead, use teh standard link format:
