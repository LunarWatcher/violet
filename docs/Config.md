...
```jsonc
{
    ...
    "prefix": null | str,
    // Whether or not to include metadata with the build, meaning information about the build itself.
    // If true, `{build output}/_violet-meta.json` is added to the build. See its schema spec for more information.
    "metadata": false,
