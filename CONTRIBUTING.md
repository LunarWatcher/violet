...
### The built-in themes are not open for additional themes or major features

Though additional themes may be added in the future, trying to maintain them all is an exercise in futility, and may add a significant maintainer overhead that I don't want to deal with. The crushing weight of themes is one of the reasons a later-sloppified zsh framework ended up refusing them as well.

The `_default` theme is explicitly designed to be minimal, to be the bare minimum required to get a site rendered without first having to set up all the things:tm: and allow for incremental changes for existing sites off a blank site without having everything ready in advance.
...
If you're not sure how to write the specific tests you want, or you aren't sure what needs testing, you're encouraged to ask.
